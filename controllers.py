...
class ControllerOptimalPredictive:
    ...
    def __init__(self,
        ...
        self.N_CTRL = N_CTRL(ctrl_bounds)
...
class StanleyController:
    def __init__(self, k_e, k_d, k_v, target_path):
        self.k_e = k_e  # Cross-track error gain
        self.k_d = k_d  # Heading error gain
        self.k_v = k_v  # Velocity gain
        self.target_path = target_path  # Target path (e.g., waypoints)

    def compute_action(self, observation):
        # Extract current position and orientation from observation
        x, y, theta = observation

        # Find the nearest point on the path to the vehicle
        nearest_point_idx = self.find_nearest_point(x, y)

        # Compute cross-track error (lateral distance to the path)
        e = self.compute_cross_track_error(x, y, nearest_point_idx)

        # Compute heading error (orientation error w.r.t. path)
        theta_path = np.arctan2(self.target_path[nearest_point_idx][1] - self.target_path[nearest_point_idx - 1][1],
                                 self.target_path[nearest_point_idx][0] - self.target_path[nearest_point_idx - 1][0])
        theta_error = theta - theta_path

        # Compute desired steering angle using Stanley control law
        delta = theta_error + np.arctan(self.k_e * e / (self.k_v + observation[3]))

        return delta

    def find_nearest_point(self, x, y):
        # Implement logic to find the nearest point on the path to the vehicle
        pass

    def compute_cross_track_error(self, x, y, nearest_point_idx):
        # Implement logic to compute cross-track error
        pass
