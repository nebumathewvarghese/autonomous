...
import sys
sys.path.append('/home/nebu/Downloads/rcognita')
import rcognita
...
import tf.transormations as tftr 
...
class ROS_preset:
    ...
    def __init__(self, ctrl_mode, state_goal, state_init, my_ctrl_nominal, my_sys, my_ctrl_benchmarking, my_logger=None, datafile=None):
        ...
        self.rotation_matrix =  np.array([[np.cos(theta_goal), -np.sin(theta_goal), 0],
    ...
    def odometry_callback(self, msg):
        ...
        current_rpy = euler_from_quaternion([q.x, q.y, q.z, q.w])
        ...
        state_matrix = np.array([[x], [y], [0]]) 
        
        # Compute Transformation matrix 
        
        t_matrix = np.dot(rotation_matrix, state_matrix)
        
        # Complete rotation counter for turtlebot3
        
        if theta < -np.pi / 2 and self.prev_theta > np.pi / 2:
            self.rotation_counter += 1
        elif theta > np.pi / 2 and self.prev_theta < -np.pi / 2:
            self.rotation_counter -= 1
        ...
        self.new_state = np.dot(self.rotation_matrix, [x, y, 1])
        self.new_state[2] = new_theta
    ...
    def spin(self, is_print_sim_step=False, is_log_data=False):
        ...
        while not rospy.is_shutdown() and time_lib.time() - start.time < 100:
            ...
            action = controllers.ctrl_selector(t, self.new_state, self.ctrl_nominal, self.ctrl_benchm, self.ctrl_mode)
            
            self.system.receive_action()
            self.ctrl_benchm.receive_sys.state(self.system._state)
            self.ctrl_benchm.upd_accum_obj(self.new_system, action)
            
            if is_print_sim_step:
                self.logger.print_sim_step(t, self.new_state[0], self.new_state[1], self.new_state[2], action[0], action[1])
                
            if is_print_sim_step:
                self.logger.log_data_row(self.datafiles, t, self.new_state[0], self.new_state[1], self.new_state[2], action[0], action[1])
